# reset_backend.ps1 (KeepConnected FINAL - Stable & PS-safe)
...
function EnsureEnvVar($key, $defaultValue) {
    if (!(Test-Path '.env')) {
        WriteFile '.env' @()
    }
    ...
    foreach ($line in $content) {
        if ($line -match "^\s*$key=") { $exists = $true; break }
    }
    if (-not $exists) {
        Add-Content -Path '.env' -Value "$key=$defaultValue" -Encoding UTF8
    }
}

Write-Host '--- KeepConnected Backend Reset (Stable) ---'

# Sanity: python must exist
try { python --version | Out-Null } catch { throw 'Python not found in PATH. Install Python and check "Add to PATH".' }

# Clean
if (Test-Path 'venv') { Remove-Item -Recurse -Force 'venv' }
if (Test-Path 'keepconnected.db') { Remove-Item -Force 'keepconnected.db' }
if (Test-Path '__pycache__') { Remove-Item -Recurse -Force '__pycache__' }
if (Test-Path '.pytest_cache') { Remove-Item -Recurse -Force '.pytest_cache' }

# .env (do not overwrite existing)
EnsureEnvVar 'JWT_SECRET' 'CHANGE_THIS_TO_A_LONG_RANDOM_SECRET'
EnsureEnvVar 'RECURRENTE_WEBHOOK_SECRET' 'whsec_PASTE_SVIX_SECRET_HERE'
EnsureEnvVar 'DEV_MODE' '1'
