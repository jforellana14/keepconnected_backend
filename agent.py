...
BACKEND = "https://api.keepconnected.io"  # o localhost si pruebas local
...
BACKEND = "https://api.keepconnected.io"

def wait_until_plan_active(token):
    while True:
        user = get_user(token)
        if user and plan_is_active(user):
            print(f"✅ Plan activo: {user['plan']}")
            return user
        else:
            print("⛔ Plan inactivo. Esperando renovación...")
            time.sleep(600)  # reintenta cada 10 minutos
...
def notify_plan_expired():
    notification.notify(
        title="KeepConnected",
        message="Tu plan ha vencido. Renueva para continuar enviando mensajes.",
        timeout=10
    )

def plan_is_active(user_data):
    return user_data.get("plan") is not None
...
def validate_user(token):
    ...
    if not data["plan"]:
        print("❌ No tienes plan activo")
        sys.exit()

    print(f"✅ Plan activo: {data['plan']}")
    return data
...
def get_limits(token):
    ...
    if r.status_code != 200:
        print("❌ No se pudieron obtener límites")
        sys.exit()
